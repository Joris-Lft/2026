...
export async function createMeasure(
  ...
  try {
    ...
    const measure: Measure = {
      id: record.id,
      date: record.fields[AIRTABLE_MEASURES_DATE_FIELD] as ISOStringFormat,
    ...
    return {
      measure: null,
      error: error?.message || "Erreur lors de la création de la mensuration",
    };
  }
}
 ...
 * @returns Liste des mensurations de l'utilisateur, triées par date décroissante
 */
...
export async function updateMeasure(
  ...
  try {
    ...
    return {
      measure: null,
      error:
        error?.message || "Erreur lors de la mise à jour de la mensuration",
    };
  }
}
...
export async function deleteMeasure(
  ...
  try {
    ...
    return {
      success: false,
      error:
        error?.message || "Erreur lors de la suppression de la mensuration",
    };
  }
}
